<?php
class ProductIntoBag extends Connection
{
    ...
    public function productIntoBag()
    {
        try {
            $productsArr = [];
            ...
            if ($selectUserIdQuery->num_rows) {
                ...
                $suerId = $row['id'];
                $productIdIntoCartQuery = $this->connection->query("SELECT product_id from bags where user_id = $suerId");
                ...
                if ($productIdIntoCartQuery->num_rows) {
                    ...
                    $this->status = 200;
                    $this->message = "Fetch all products of the bag";
                } else {
            ...
            error("ProductIntoBag.php", $error->getCode(), $error->getMessage(), $error->getLine());
        } finally {
            $response = [
                'status' => $this->status,
